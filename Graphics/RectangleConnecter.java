...
class ConnectionCheck{
	...
	int[][][] linepoints={{{13,13},{38,55}},{{13,13},{40,90}},{{38,55},{40,90}},{{40,90},{75,68}},{{120,25},{38,55}},{{75,68},{13,13}},{{120,25},{160,25}},{{13,13},{200,200}}};
	...
	public void checkconnection(){
		...
		for (int i = 0; i < linepoints.length; i++) {
			...
			for (int k = 0; k < rectangles.length; k++) {
				...
				if (temprectk.contains(linepoints[i][1][0],linepoints[i][1][1])) {
					connectors[i][1]=k+1;
				}//close if
			}//close for k
		}//close for
		...
		for (int i = 0; i < connectors.length; i++) {
			System.out.println("[DEBUG] van blok " + connectors[i][0]+ " naar blok " + connectors[i][1]);
		...
		no_connectors = new int[no_connection][3];
		for (int i = 0; i <connectors.length; i++) {
			if (connectors[i][0]!=0 && connectors[i][1]!=0) {
