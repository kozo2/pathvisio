...
/* Force calculations ready. Method "calculateDisplacement" added.
...
/* Method to calculate block displacements */
...
public static int[][] calculateDisplacement(double[][] forces, int displacementStepSize ) {
	...
	//Rounding to integers and taking displacementStepSize into account to calculate displacement
...
//Program code: tryout 
  ...
  for(nRow=0; nRow<(electricalForces.length); nRow++) {
		for(nColumn=0; nColumn<(coord.length); nColumn++) {
	...
	int[][] displacement = calculateDisplacement(forces, 10);
	
	//Updating coord to newCoord
	int[][] newCoord = new int [coord.length][coord[0].length];
	
	for(nRow=0; nRow<(coord.length); nRow++) {
		for(nColumn=0; nColumn<(coord[0].length); nColumn++) {
			coord[nRow][nColumn]= (coord[nRow][nColumn])+displacement[nRow][nColumn]
			}
		}
		
	/* THE ABOVE CODE CALCULATES ONLY ONE DISPLACEMT; THIS SHOULD BE TRANSFORMED INTO A LOOP THAT STOPS
		AS SOON AS ALL FORCES DROP BELOW A CERTAIN TRHESHOLD /*
  }
}	
